    ...
    <v-responsive class="align-center mx-auto" v-else-if="status == 'select_metrics'">
      ...
      <v-row no-gutters class="mb-3">
        <v-col no-gutters cols="10" class="pr-0">
          <metrics :participants="hasParticipants ? participants : []" :game=game :isProblemA="game?.currentProblem?.id == game?.problemA?.id" :metricsGroup="twoMetricsGroupsSelected" :problem="problem" :avatars="choosenByParticipants" :isDealer="isDealer" ></metrics>
        </v-col>
        <v-col no-gutters cols="2" class="d-flex pl-2 align-center justify-end">
          <participants :gameId="id" @input="setParticipants"></participants>
        </v-col>
